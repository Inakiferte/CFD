#=======================================
# Code for different Reynolds numbers
...
nu = [1.0/40.0, 1.0/40.0, 1.0/40.0, 1.0/40.0]                         # Kinematic viscosity
i  = 1j                                   # Complex number i

#=======================================
# Numerical input data
#=======================================
N        = [20,20,20,20]                  # Vector length
Ck       = [0.4,0.3, 0.2, 0.1]            # Ck parameter
...
for Z in range(len(N)):
    ...
    for time in range(time_max):
        ...
        for k in range(2, N[Z]):
            ...
            if LES == 0:
                uk[k] = uk_zero[k] - delta_T * (nu[Z] * k**2 * uk_zero[k] + conv)
            elif LES == 1 :
    ...
    if Z == 0 and LES == 1:
        ...
        k_cv3 = np.linspace(1, N[0], N[0])               # Generate k points with the same spacing
        Ek3   = np.zeros(N[0])
        Ek3   = Ek
    else:
        k_cv4 = np.linspace(1, N[1], N[1])               # Generate k points with the same spacing
        Ek4 = np.zeros(N[1])
        Ek4 = Ek
...
# Starting plots
#=======================================
output_plot = f"Results/Burguer_kconstant.pdf"
...
# Plot
plt.loglog(k_cv1[1:], Ek1[1:],'-x', color='red', label=r"$C_{k}=0.4$")
plt.loglog(k_cv2[1:], Ek2[1:], '-o', color='black', label=r"$C_{k}=0.3$")
plt.loglog(k_cv3[1:], Ek3[1:],'-+', color='blue', label=r"$C_{k}=0.2$")
plt.loglog(k_cv4[1:], Ek4[1:], '-*', color='green', label=r"$C_{k}=0.1$")
