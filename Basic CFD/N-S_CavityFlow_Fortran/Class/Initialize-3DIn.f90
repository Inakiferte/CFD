!----------------------------------------------------
!This program plots a 3D inhomogeneous cube         -
!gfortran tipos.f90 Initialize-3DIn.f90             -
!.a.out                                             -
!xmakemol -f fort.3                                 -
!----------------------------------------------------
...
character*2, parameter :: ELEMN= 'N'
character*2, parameter :: ELEMO= 'O'
 ...
 do w=1, Ny
  do k=1, Nz
   ...
   if (MOD(ITER,2) .EQ. 0) then
    write(3,'(a2,3F20.10)') ELEMN, X(i), Y(w), Z(k)
   else 
    write(3,'(a2,3F20.10)') ELEMO, X(i), Y(w), Z(k)
   end if
  end do
 end do
end do
